...
const DERIVATION = [
  {
    ...
    items: [
      ...
      { labels: ["為替差損益"], requires: ["fxEffect"],
        inputs: [{ path: "detail.fxEffect", sign: -1 }],
        note: "換算差額は税引前当期純利益に含まれているため営業から消去し、同額を「現金及び現金同等物に係る換算差額」として3区分の外に置きます。" },
      ...
      { labels: ["その他流動資産の増加額", "その他流動資産の減少額",
                 "その他流動資産の増加額(未収利息を除く)", "その他流動資産の減少額(未収利息を除く)"],
      ...
      { labels: ["その他流動負債の増加額", "その他流動負債の減少額",
                 "その他流動負債の増加額(未払利息・未払法人税等を除く)", "その他流動負債の減少額(未払利息・未払法人税等を除く)"],
  ...
  },
  {
    ...
    items: [
      ...
      { labels: ["定期預金の預入による支出", "定期預金の払戻による収入"],
        inputs: [{ path: "bs.curr.cashExcluded", sign: -1 }, { path: "bs.prev.cashExcluded", sign: 1 }],
        note: "現金同等物に含めない預金への振替は投資活動になります。この行がある分だけ増減額そのものが変わります。" },
    ],
  },
  {
    id: "financing",
  ...
  },
  {
    ...
    items: [
      { labels: ["現金及び現金同等物に係る換算差額", "Ⅳ 現金及び現金同等物に係る換算差額"], requires: ["fxEffect"],
        inputs: [{ path: "detail.fxEffect", sign: 1 }],
        note: "3区分の外に置く独立の行です。営業から消去した同額をここに戻すため、増減額は変わりません。" },
      { labels: ["Ⅳ 現金及び現金同等物の増減額", "Ⅴ 現金及び現金同等物の増減額"],
        computed: "営業CF + 投資CF + 財務CF + 換算差額" },
      { labels: ["Ⅴ 現金及び現金同等物の期首残高", "Ⅵ 現金及び現金同等物の期首残高"],
        inputs: [{ path: "bs.prev.cash", sign: 1 }, { path: "bs.prev.cashExcluded", sign: -1 }],
        note: "現金同等物 = 現金及び預金 − 現金同等物に含めない預金。" },
      { labels: ["Ⅵ 現金及び現金同等物の期末残高", "Ⅶ 現金及び現金同等物の期末残高"],
        computed: "期首残高 + 増減額" },
    ],
  },
];
...
const DETAIL_GROUPS = [
  ...
  },
  {
    ...
    effect: "小計より後の行が実額に置き換わります。発生額との差(未収利息・未払利息・未払法人税等の増減)は、その他流動資産／負債の増減の行から取り除いて二重計上を防ぎます。営業CFの合計は変わりません。",
  },
];
